import { isInCategory } from '#engine/events/category'
import type { EventCategoryBits } from '#engine/events/category'
...
//   - on(type, h):         fires for events whose `type` string matches exactly
...
export class EventBus {
  private readonly byType: Map<string, Set<EventHandler>> = new Map()
  private readonly byCategory: Set<EventHandler> = new Set()

  public on<E extends Event>(type: string, handler: EventHandler<E>): () => void {
  ...
  public publish(event: Event): void {
    const typeHandlers = this.byType.get(event.type)
    if (typeHandlers) {
